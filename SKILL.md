...
14. **开放列举与微量拓界 (Open-Set Expansion)**: 用户写“例如、xx 等、等等、之类、包括但不限于”时，默认把已列项视为种子而非闭集：识别上位类别和相邻成员，只补会改变当前答案的 1–3 个关键遗漏，并区分用户给出与模型补充。用户写“仅、只、严格限于、就这几项、不要扩展”时才按闭集执行。普通任务即使由其他 Skill 主路由，也可环境式运行 `cognitive-expansion` 微量扫描，补目标/对象、机制或生命周期、角色/尺度、失败/反向和遗漏中的高价值项；不单独打 Trace、不抢主路由、不默认联网。
...
| 普通任务里说“多维度/多角度/xx等/之类”，只想避免漏项 | 当前主 Skill + `cognitive-expansion` 微量辅助 | 只补 1–3 个高价值遗漏，不打 Trace、不另起报告、不因“等”自动联网；明确闭集措辞优先 |
