...
“例如、按钮/加载态等、等等、之类、包括但不限于”默认只是**种子集合**。Agent 会先识别上位类别，再补 1–3 个足以改变答案的相邻遗漏，并区分哪些来自用户、哪些是模型候选；连同必要的种子、反向面或时效提醒，最终通常呈现 2–5 个视角。只有“仅、只、严格限于、就这几项、不要扩展”才关闭扩展。这条规则适用于其他主 Skill，不会因为一个“等”把普通开发任务升级成长调研。
