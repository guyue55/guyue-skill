...
- 绝对禁止把一次性任务包装成 Loop Contract。没有稳定输入、可重复步骤、验证资产和停止条件时，只能写普通复盘或改进建议。
- 绝对禁止把阶段完成、脚本通过或截图无白屏写成最终完成；长程 SOP 必须保留 MVP、local-only、release candidate、production-ready 等状态差异。
...
5. **循环判断**：如果这条链路可能重复，补充 `Loop Contract`；如果不满足稳定输入、停止条件和验证资产，只说明“不适合包装为循环”。
6. **长程资产抽取**：如果任务跨阶段，补齐总控文档字段、执行账本字段、阶段门禁、活体证据、否定清单和恢复顺序。
7. **模板输出**：严格套用 SOP 模板。
