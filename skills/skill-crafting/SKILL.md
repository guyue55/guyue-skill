---
name: skill-crafting
description: 古月视角的“技能制作、技能打磨、技能接入与发布候选门禁”大师法则（自带主动探矿雷达）。用于制作新 Skill、升级现有 Skill、提取心智、挖掘本地会话语料、使用 /huashu-nuwa 或 /luban、把外部 Skill 接入 Codex、本地验证技能是否被加载、规划 release-candidate 证据包、做零泄漏发布检查时触发。
---
...
1. **先蒸馏，后打磨 (Distill First, Polish Later)**
   - 严禁拍脑袋写 Prompt。任何高质量技能的诞生，必须先从真实的、多轮的历史会话（如 `~/.codex/sessions/` 或 `<appDataDir>/brain/` 等本地 Agent 会话日志目录）中提取“活体行为”；或者吸收 `cangjie-skill` 的长文提纯心智，从宏大的外部文本/视频资料中提取结构化知识。
   - 使用 `/huashu-nuwa` (女娲) 剥离出业务表象背后的“心智模型”、“思考链路”与“处理方案”。若处理长文本，应使用类似“RIA-TV++”拆解框架的阅读理解管道。
...
4. **活体检测与闭环进化 (Live Testing & Recursive Refinement)**
   ...
   - 若技能用于长任务、发布候选、审计或多阶段执行，必须设计执行账本、否定清单和证据新鲜度检查；否则 Agent 会在上下文压缩或旧证据下误报完成。
5. **工具组合编排 (Tool Orchestration)**
   - 擅长将不同工具组合使用。把大任务拆解，用专门的工具做专门的事。例如，数据清洗交由脚本提取，心智萃取交由女娲，规则落地交由鲁班。
6. **循环工程封装 (Loop Engineering Packaging)**
   - 不因为出现“Loop Engineering”“动态工作流”“subagent”这些词就新建技能。只有当任务重复出现、输入稳定、步骤可复用、输出可验证、成本值得投入时，才把它包装为 Skill、Custom subagent、Hook、Automation 或 CI gate。
   - 每个动态工作流都必须带检查器、停止条件、最大轮数、时间/Token/子任务预算和人工检查点；没有这些边界，只能保留为 SOP 或普通计划。
...
2. **主动环境侦察 (Active Reconnaissance)**：
   - 即使设定了具体方向，Agent 必须主动运行探矿脚本扫描本地日志。
   - **执行命令**：在 Guyue 仓库根目录运行 `python3 scripts/ai_log_scanner.py`。若当前目录不是 Guyue 仓库根目录，先 `cd /path/to/guyue`。
   - 将脚本输出的矿脉菜单（包含各主流工具的日志体量和路径）完整展示给用户。
   - > [!WARNING]
     > **数据安全读取纪律 (强制黑盒提取)**：
     > Agent 严禁裸敲 SQL 读取复杂日志，以防显存爆炸。必须调用专用提取器，例如：
     > - `python3 scripts/cursor_extractor.py <db_path>`
     > - `python3 scripts/claude_extractor.py <jsonl_path>`
...
7. **长程执行审查 (Long Goal Readiness)**：如果技能可能驱动多阶段执行，必须明确：
   - 总控文档应包含哪些字段。
   - 执行账本每阶段记录什么。
   - 何时能标记阶段完成，何时能标记最终完成。
   - 哪些旧截图、旧报告、旧构建或旧服务表现不能作为当前证据。
   - 至少 1 个测试 prompt 覆盖“继续/恢复/不要误报完成”的场景。
...
### 可复用 Lane A：本地 Skill 接入验证

当用户要求“把技能目录加到本地技能目录并重载”“让 Codex 能直接使用这个 Skill”时，按本 Lane 执行：

1. 确认目标目录含有 `SKILL.md` 或一个可扫描的 `skills/` 子树。
2. 只做链接或轻量注册，不复制第三方完整源码进当前公开仓库。
3. 使用会跟随符号链接的扫描命令验证数量，例如 `find -L <skills-root> -mindepth 2 -maxdepth 2 -name SKILL.md`。
4. 使用当前 runtime 的可见 prompt/render/debug 能力确认模型确实能看到根 Skill 和代表性子 Skill。
5. 如果只看到根 Skill，看不到子技能，继续排查扫描深度、符号链接和 runtime 支持，不要把文件系统存在误报成“已加载”。

### 可复用 Lane B：发布候选与零泄漏门禁

当用户要求“产品化”“发布候选”“注意规范和边界”“暂不开发新功能，只做发布准备”时，按本 Lane 执行：

1. 先读 `SKILL.md`、`README.md`、`docs/release-checklist.md`、`docs/release-candidate.md`、`docs/runtime-adapters.md`。
2. 运行或规划本地验证门：`bash scripts/test_suite.sh`、`git diff --check`、缓存文件检查、`python3 scripts/security_scanner.py`。
3. 对 dirty worktree 做归属判断，用户已有改动不得混入本轮发布候选切片。
4. 如需提交，先确认 staged 范围；必要时用可审计的隔离方式保护用户改动。
5. 任何零泄漏、绝对路径、凭据、不可复现依赖、过度 readiness 宣称都必须写入 blocker，而不是淡化处理。
6. 不为 Claude、Gemini、Copilot、Cursor 等 runtime 预创建适配入口；只有 runtime-specific live replay 证明需要时才新增。

### 可复用 Lane C：Loop Engineering / Dynamic Workflow Packaging

当用户要求“把反复提示的任务做成 Loop Engineering”“设计动态工作流”“封装子 Agent 编排”时，按本 Lane 执行：

1. **先验料**：确认该工作至少发生过两次，或虽然次数少但明显会再次发生且重复成本高。
2. **看稳定输入**：列出每次都能拿到的输入，例如仓库、需求、日志、接口、文档、截图、测试、发布证据。
3. **拆循环体**：写出 Agent loop、Evaluator loop、Workflow loop 或 Improvement loop 的循环动作；不要把探索性闲聊写成循环。
4. **定包装形态**：
   - 稳定团队流程 -> Skill。
   - 边界明确、可委派的专家角色或调研任务 -> Custom subagent。
   - 需要自动触发或本地工具接管 -> Hook 或 Automation。
   - 需要阻止回归 -> CI gate。
5. **定预算和检查器**：写清最大轮数、最大时间、最大 Token、最大子任务数量、工具范围、人类检查点和独立验证方式。
6. **先小样本回放**：用 read-only replay 或最小样本证明流程能触发、能停止、能产出验证资产，再写入主路由或 manifest。
...
- **禁止无限动态工作流**：不得创建没有预算、没有停止条件、没有独立检查器的循环或并行子 Agent 编排。
- **禁止无账本长跑**：不要让一个 Skill 驱动多阶段任务却没有恢复入口、账本字段和完成否定清单。长任务没有账本，就会在上下文压缩后丢失边界。
