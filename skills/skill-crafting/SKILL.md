...
2. **主动环境侦察 (Active Reconnaissance)**：
   ...
   - **执行命令**：`python3 ${GUYUE_SKILLS_DIR:-~/skills/guyue}/scripts/ai_log_scanner.py`
   - 将脚本输出的矿脉菜单（包含各主流工具的日志体量和路径）完整展示给用户。
   - > [!WARNING]
     > **数据安全读取纪律 (强制黑盒提取)**：
     > Agent 严禁裸敲 SQL 读取复杂日志，以防显存爆炸。必须调用专用提取器，例如：
     > - `python3 ${GUYUE_SKILLS_DIR:-~/skills/guyue}/scripts/cursor_extractor.py <db_path>`
     > - `python3 ${GUYUE_SKILLS_DIR:-~/skills/guyue}/scripts/claude_extractor.py <jsonl_path>`
