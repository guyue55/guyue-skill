...
5. **绝对真实性体检 (Exhaustive Truth Audit)**：拒绝表面上的自圆其说。必须通过**物理编写和运行 Python 探针**（如 `exhaustive_truth_auditor.py`），全量、深度遍历技能文件。严格校验：是否存在任何占位符 (`pass`, `...`)、编译级错误、或诸如 "等相关内容"、"具体的"、"xxx" 这种 AI 偷懒黑话。如果探针报出任何一处 `[AI Laziness]` 或 `[Unimplemented Code]`，立刻打回重构，绝不姑息。
