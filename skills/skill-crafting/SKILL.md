...
2. **主动环境侦察 (Active Reconnaissance)**：
   ...
   - **执行命令**：`python3 ~/skills/guyue-perspective/scripts/ai_log_scanner.py`
   ...
   - > [!WARNING]
     ...
     > - **Cursor** 的 SQLite 数据库 (`state.vscdb`)：必须调用 `python3 ~/skills/guyue-perspective/scripts/cursor_extractor.py <db_path>`。
     > - **Claude Code** 的海量 JSONL 日志：必须调用 `python3 ~/skills/guyue-perspective/scripts/claude_extractor.py <jsonl_path>`。
...
5. **环境脱敏审查 (De-hardcoding Audit)**：全面扫描生成的技能内容和配套脚本，绝不允许出现特定用户的绝对硬编码路径（如 `/Users/apple/...`），必须使用相对路径、泛化路径符（如 `~`、`<appDataDir>`）或环境变量，以保证技能跨机器复用的兼容性。
...
2. 获得授权后，将 `SKILL.md` 写入本地自定义技能目录（例如 `~/skills/...` 或用户的 Customization Root 目录）。
3. 在主干路由技能（如 `guyue-perspective/SKILL.md`）中为其增加路由入口。
4. （重要）提示用户需要进行一次完整的 Mock 验证。

## 班规戒律 (Anti-Patterns Blacklist)
- **禁止无语料生造**：没有经过真实会话提取就直接写技能，视为事故。
- **禁止越权部署**：未经明确祈使句授权，严禁直接将技能文件写盘或覆盖现有路由资产。
- **禁止硬编码环境绑定 (No Hardcoded Paths)**：技能及其关联脚本必须是环境无关的。严禁在代码、配置和 Markdown 中写死特定机器或用户的绝对路径（如 `/Users/apple/...`），一旦发现视为严重违规，打回重造。
- **禁止“大全式”技能**：违背 Single Responsibility（单一职责）。一个技能只解决一类具体的专业问题，如果试图把多个不相关的任务揉在一个 Skill 里，立刻拆分并利用主路由进行多技能编排。
