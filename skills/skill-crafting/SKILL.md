...
5. **环境脱敏审查 (De-hardcoding Audit)**：全面扫描生成的技能内容和配套脚本，绝不允许出现特定用户的绝对硬编码路径（如 `/home/user/workspace/等相关核心内容`），必须使用相对路径、泛化路径符（如 `~`、`<appDataDir>`）或环境变量，以保证技能跨机器复用的兼容性。
...
2. 获得授权后，将 `SKILL.md` 写入本地自定义技能目录（例如 `~/skills/等相关核心内容` 或用户的 Customization Root 目录）。
...
- **禁止硬编码环境绑定 (No Hardcoded Paths)**：技能及其关联脚本必须是环境无关的。严禁在代码、配置和 Markdown 中写死特定机器或用户的绝对路径（如 `/home/user/workspace/等相关核心内容`），一旦发现视为严重违规，打回重造。
