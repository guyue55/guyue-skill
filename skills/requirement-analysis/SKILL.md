...
4. **目的高于手段，判断高于顺从**
   - 用户对价值、优先级和不可接受结果拥有决定权；用户提出的实现手段、阶段命名和完成口径仍需接受专业审查。
   - 发现手段与目的冲突时，必须给出当前事实、实际代价、推荐替代和什么证据会推翻建议。不能盲从，也不能为显示独立而无证据唱反调。
...
- 当用户提出“我想做个新功能/页面/模块”或“给当前项目做一个……”，但只给了粗略描述时。
...
2. **Phase 2: 启发式拷问 (Iterative Interrogation)**
   ...
   - 即使方向问题尚未关闭，也要基于当前默认假设给出一组**暂定、待确认**的验收口径，至少覆盖核心价值、权限/安全、失败回退和留痕；不能因为还要追问就完全省略“如何验证”。
   - `P0/P1/P2` 只表示业务优先级；首期必备范围写作“首期必须项”，不得复用 P0 造成范围与优先级冲突。
...
4. **Phase 4: 输出收敛契约 (Converge to Contract)**
   ...
   - **验收标准 (Acceptance Criteria)**: [价值结果、权限/安全、异常回退和留痕如何验证]
   - **ROI 优先级**: [P0/P1/P2]
...
调查还要做一次“上浮/下钻”检查：向上区分终极愿景、本 Goal 可交付状态和必须靠时间验证的结果；向下寻找最危险、最能代表用户价值的纵向切片。只有宏大愿景没有可证伪切片，或只有任务清单没有上层价值，都不算需求关闭。
...
- 权威顺序、当前控制基线、旧完成口径是否撤销，以及哪些新事实会推翻控制包；
- 已验证事实、冻结决定、待证假设和差异化实验，以及先纵切后扩张的风险门。
...
决策关闭后，将需求契约交给 Guyue 根路由的 Long Goal Forge，由它按 `docs/long-goal-protocol.md` 和控制包模板生成总控文档、执行账本、阶段计划与证据索引。总控必须逐个列出全部阶段计划文件，并通过 `python3 <guyue-root>/scripts/check_long_goal_pack.py --repo-root <target-repo> <goal-master.md>`；准备资产通过独立就绪审查后，才允许输出一行执行入口。
