...
| Skill registry | `skills_manifest.json` records 26 routed skills; `test-prompts.json` contains 41 structural prompts. | pass | Keep manifest, README, and tests synchronized when adding skills. |
...
- Later human-voice integration expanded the manifest to 26 routed skills and `test-prompts.json` to 41 structural prompts, but release evidence still carried older count snapshots.
...
- Updated release evidence and checklist counts again to reflect the current 26 routed skills and 41 structural prompts after the human-voice gate was integrated.
